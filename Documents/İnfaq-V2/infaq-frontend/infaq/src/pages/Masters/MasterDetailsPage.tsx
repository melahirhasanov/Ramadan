...
const MasterDetailsPage: React.FC = () => {
  ...
  const handleDelete = async () => {
    if (!window.confirm('Bu ustadı silmək istədiyinizdən əminsiniz?')) return;
    try {
  ...
  if (!person) return <div className={styles.empty}>Ustad tapılmadı</div>;
    ...
    <>
      ...
      <div className={styles.container}>
        <div className={styles.detailHeader}>
          <Link to="/masters" className={styles.backBtn}>
            <FaArrowLeft /> Geri
          </Link>
          <div>
            <Link to={`/masters/edit/${person._id}`} className={styles.editBtn}>
              <FaEdit /> Redaktə et
            </Link>
